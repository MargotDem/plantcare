"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
var plant_1 = __importDefault(require("../databaseLayer/plant"));
var user_1 = __importDefault(require("../databaseLayer/user"));
var databaseCallBack_1 = __importDefault(require("./databaseCallBack"));
var Plant = new plant_1.default("plants");
var User = new user_1.default("users");
var plantsController = {
    getScheduledPlantsByUserId: function (req, res) {
        var id = parseInt(req.params.user_id);
        var callBack = (0, databaseCallBack_1.default)(function (results) { return res.status(200).json(results.rows); }, function (_error) {
            return res.status(500).json({ message: "Database error" });
        });
        Plant.getByUserIdAsc(id, callBack);
    },
    getPlantsByUserId: function (req, res) {
        var id = parseInt(req.params.user_id);
        var callBack = (0, databaseCallBack_1.default)(function (results) { return res.status(200).json(results.rows); }, function (_error) {
            return res.status(500).json({ message: "Database error" });
        });
        Plant.getByUserId(id, callBack);
    },
    createPlant: function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
        var newPlantId, user_id;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    newPlantId = 0;
                    return [4 /*yield*/, new Promise(function (resolve, _reject) {
                            var callBack = (0, databaseCallBack_1.default)(function (results) {
                                newPlantId = results.rows[0].id;
                                resolve();
                            }, function (_error) {
                                return res.status(500).json({ message: "Database error" });
                            });
                            Plant.createPlant(req.body, callBack);
                        })];
                case 1:
                    _a.sent();
                    user_id = parseInt(req.params.user_id);
                    return [4 /*yield*/, new Promise(function (resolve, _reject) {
                            var callBack = (0, databaseCallBack_1.default)(function (_results) {
                                resolve();
                            }, function (_error) {
                                return res.status(500).json({ message: "Database error" });
                            });
                            Plant.assignUserToPlant(user_id, newPlantId, callBack);
                        })];
                case 2:
                    _a.sent();
                    // send response
                    res.status(201).json({ message: "Plant added with ID: ".concat(newPlantId) });
                    return [2 /*return*/];
            }
        });
    }); },
    assignUserToPlant: function (req, res) {
        var user_id = parseInt(req.params.user_id);
        var plant_id = parseInt(req.params.plant_id);
        var callBack = (0, databaseCallBack_1.default)(function (results) { return res.status(200).json(results.rows); }, function (_error) {
            return res.status(500).json({ message: "Database error" });
        });
        Plant.assignUserToPlant(user_id, plant_id, callBack);
    },
    getPlantById: function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
        var id, plantInfo;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    id = parseInt(req.params.id);
                    plantInfo = {
                        plant: null,
                        users: [],
                    };
                    return [4 /*yield*/, new Promise(function (resolve, _reject) {
                            var callBack = (0, databaseCallBack_1.default)(function (results) {
                                plantInfo.plant = results.rows[0];
                                resolve();
                            }, function (_error) {
                                return res.status(500).json({ message: "Database error" });
                            });
                            Plant.getById(id, callBack);
                        })];
                case 1:
                    _a.sent();
                    return [4 /*yield*/, new Promise(function (resolve, _reject) {
                            var callBack = (0, databaseCallBack_1.default)(function (results) {
                                plantInfo.users = results.rows;
                                resolve();
                            }, function (_error) {
                                return res.status(500).json({ message: "Database error" });
                            });
                            User.getByPlantId(id, callBack);
                        })];
                case 2:
                    _a.sent();
                    res.status(200).json(plantInfo);
                    return [2 /*return*/];
            }
        });
    }); },
    deletePlant: function (req, res) {
        var id = parseInt(req.params.id);
        var callBack = (0, databaseCallBack_1.default)(function (_results) {
            res.status(200).json({ message: "Plant deleted with ID: ".concat(id) });
        }, function (_error) {
            return res.status(500).json({ message: "Database error" });
        });
        Plant.delete(id, callBack);
    },
    updatePlant: function (req, res) {
        var id = parseInt(req.params.id);
        var callBack = (0, databaseCallBack_1.default)(function (_results) {
            res.status(200).json({ message: "Plant updated with ID: ".concat(id) });
        }, function (_error) {
            return res.status(500).json({ message: "Database error" });
        });
        Plant.updatePlant(id, req.body, callBack);
    },
};
exports.default = plantsController;
